...
## The pipeline (the only legal ship path)
    ...
    era-data/<era>/<class>.yaml            <- the single source of truth
        ...
        +-> data/sql/world/base/*.sql   (spell_dbc, spell_proc,
        |     spell_script_names, era_talent, era_talent_rank, era_talent_meta stamp)
        +-> client-addon/EraTalents/data/generated/*.lua   (addon tree + spellMods)
        +-> client-addons/_data-patches/patch-V.mpq             (client Spell.dbc +
              SkillLineAbility rows + sentinel 932999, merged into IP's patch)

    Ship server side: the module dir under <ac>/modules/ (a clone, or era-regen --sync-fork <ac> for a
    checkout elsewhere) -> the core DB updater applies SQL -> worldserver restart.
...
| `client-addon/EraTalents/build-addon.sh` | manifest-driven (`era-data/datasets.txt`) `gen_era_talents.py --lua` per dataset |
| `client-addon/EraTalents/EraTalents.toc` | the generated `data\generated\<Class>Vanilla.lua` line — miss it and the addon loads without that class, silently |
...
- **`src/EraTalentProcScripts.cpp` has ~5 near-identical band-DUMMY-marker
