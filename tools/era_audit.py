...
def _find_ac_root():
    """AzerothCore root for the OPTIONAL cross-checks (SpellInfoCorrections scan, base trainer SQL):
    $AC_ROOT, else ../.. when this repo is installed as <ac>/modules/mod-era-talents, else None
    (those checks are skipped, as they already are on a checkout without the core)."""
    env = os.environ.get("AC_ROOT")
    if env:
        return pathlib.Path(env)
    cand = _TOOLS.parent.parent.parent
    return cand if (cand / "src" / "server").is_dir() else None


_AC_ROOT = _find_ac_root()
...
# None when no AzerothCore root is resolvable — _spellfix_sets() then returns [] (skip path).
_SPELLFIX_SRC = (None if _AC_ROOT is None else
                 _AC_ROOT / "src" / "server" / "game" / "Spells" / "SpellInfoCorrections.cpp")
...
def _spellfix_sets():
    ...
    Returns [] when no AzerothCore root is resolvable (same convention as
    check_trainer_wrapper_leaks) so the audit still runs on a standalone checkout of this repo;
    set $AC_ROOT, or install the repo as <ac>/modules/mod-era-talents, to enable the check."""
    if _SPELLFIX_SRC is None or not _SPELLFIX_SRC.exists():
        return []
...
# no AzerothCore root is resolvable (see _find_ac_root) or its base trainer dump is absent.
...
_MODULE_SQL_DIR = _TOOLS.parent / "data" / "sql" / "world" / "base"
# None when no AzerothCore root is resolvable — _effective_trainer_rows() then returns
# (None, None) and every trainer cross-check is skipped, same as an absent dump.
_BASE_TRAINER_SQL = (None if _AC_ROOT is None else
                     _AC_ROOT / "data" / "sql" / "base" / "db_world" / "trainer_spell.sql")
...
def _effective_trainer_rows(findings: list):
    ...
    if _BASE_TRAINER_SQL is None or not _BASE_TRAINER_SQL.is_file():
        return None, None
...
def check_band_ownership(findings: list):
    ...
    cpp = "\n".join(p.read_text() for p in (_TOOLS.parent / "src").glob("*.cpp"))
    for i, e in enumerate(al.entries, 1):
...
def main(argv: list[str]) -> int:
    if _AC_ROOT is None:
        # Say so out loud: a silently-skipped cross-check reads exactly like a passing one.
        print("era_audit NOTE: no AzerothCore root (set $AC_ROOT, or install this repo as "
              "<ac>/modules/mod-era-talents) — the SpellInfoCorrections and trainer-leak "
              "cross-checks are SKIPPED", file=sys.stderr)
