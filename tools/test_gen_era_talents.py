...
def test_committed_meta_sql_matches_datasets():
    ...
    needs no edit here."""
    sql = pathlib.Path(
        "data/sql/world/base/2026_08_14_11_era_talent_meta.sql"
    ).read_text()
...
def test_addon_build_script_reads_manifest():
    """client-addon/EraTalents/build-addon.sh must derive its Lua legs from era-data/datasets.txt,
    not carry a hand list (a new dataset was silently missing from the addon otherwise)."""
    sh = pathlib.Path("client-addon/EraTalents/build-addon.sh").read_text()
    assert "era-data/datasets.txt" in sh
    assert "python3 tools/gen_era_talents.py --sql" not in sh   # no hand-listed SQL legs


def test_merge_script_reads_manifest():
    sh = pathlib.Path("client-patch/merge-into-patch.sh").read_text()
    assert "era-data/datasets.txt" in sh
    assert 'DATASETS=("$ROOT/era-data/vanilla/mage.yaml"' not in sh
